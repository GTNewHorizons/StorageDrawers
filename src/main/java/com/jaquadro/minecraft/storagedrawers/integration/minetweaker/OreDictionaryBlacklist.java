...
public class OreDictionaryBlacklist {
    ...
    public static void add(String name) {
        ...
        else MineTweakerAPI.logError("Tried to add empty ore dictionary name to drawer conversion blacklist.");
    }

    @ZenMethod
    public static void addPrefix(String prefix) {
        if (prefix != null && prefix.length() > 0) MineTweakerAPI.apply(new AddPrefixAction(prefix));
        else MineTweakerAPI.logError("Tried to add empty ore dictionary prefix to drawer conversion blacklist.");
    }
    ...
    private static class AddNameAction implements IUndoableAction {
        ...
        @Override
        public void undo() {
            if (added) StorageDrawers.oreDictRegistry.removeBlacklist(name);
        }

        @Override
        public String describe() {
            if (added) return "Adding ore dictionary key '" + name + "' to drawer conversion blacklist.";
            else return "Ore dictionary key '" + name + "' already registered in drawer conversion blacklist.";
        }
    ...
    private static class AddPrefixAction implements IUndoableAction {
        ...
        @Override
        public void undo() {
            if (added) StorageDrawers.oreDictRegistry.removeBlacklistPrefix(name);
        }

        @Override
        public String describe() {
            if (added) return "Adding ore dictionary prefix '" + name + "' to drawer conversion blacklist.";
            else return "Ore dictionary prefix '" + name + "' already registered in drawer conversion blacklist.";
        }
