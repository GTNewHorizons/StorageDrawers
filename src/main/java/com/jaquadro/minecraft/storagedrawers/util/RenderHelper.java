...
public class RenderHelper {
    ...
    private void setupColorMult(int face, IBlockAccess blockAccess, Block block, int x, int y, int z, float r, float g,
        ...
        if (blockAccess == null) {
            ...
            switch (face) {
                case YNEG:
                    brightY = (state.renderMinY > 0) ? y : y - 1;
                    break;
                case YPOS:
                    brightY = (state.renderMaxY < 1) ? y : y + 1;
                    break;
                case ZNEG:
                    brightZ = (state.renderMinZ > 0) ? z : z - 1;
                    break;
                case ZPOS:
                    brightZ = (state.renderMaxZ < 1) ? z : z + 1;
                    break;
                case XNEG:
                    brightX = (state.renderMinX > 0) ? x : x - 1;
                    break;
                case XPOS:
                    brightX = (state.renderMaxX < 1) ? x : x + 1;
                    break;
            }
