...
public class ItemFramingTable extends ItemBlock {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!world.isAirBlock(x + xOff, y, z + zOff) || !world.isAirBlock(x + xOff, y + 1, z + zOff)) {
            if (side == 2 || side == 3) xOff *= -1;
            if (side == 4 || side == 5) zOff *= -1;
            if (!world.isAirBlock(x + xOff, y, z + zOff) || !world.isAirBlock(x + xOff, y + 1, z + zOff)) return false;
            metadata = 8;
        }

        if (!world.setBlock(x, y, z, field_150939_a, metadata | side, 3)) return false;
