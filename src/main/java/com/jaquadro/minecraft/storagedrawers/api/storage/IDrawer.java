...
public interface IDrawer {
     ...
     * should not be modified for any reason. Make a copy if you need to store or modify it.
     */
    ItemStack getStoredItemPrototype();

    /**
     * Gets an ItemStack initialized to the number of items stored in this drawer. The returned ItemStack is guaranteed
     * to be a new copy and can be used for any purpose. Does not affect drawer contents.
     */
     ...
     * @param itemPrototype An ItemStack representing the type, metadata, and tags of the item to store.
     * @param amount        The amount to initialize the stored item count to.
     * @return The IDrawer actually set with the prototype. Some drawer groups can redirect a set operation to another
     *         member.
     */
    ...
    /**
     * Sets the number of items stored in this drawer. Triggers syncing of inventories and client data. Setting a
     * drawer's count to 0 may also result in the item type being cleared, depending in implementation.
     *
    ...
    /**
     * Gets the maximum number of items that can be stored in this drawer. This value will vary depending on the max
     * stack size of the stored item type.
     */
    int getMaxCapacity();
     ...
     *
     * Stack size and available capacity are not considered. For drawers that are not empty, this method can allow
     * ore-dictionary compatible items to be accepted into the drawer, as defined by what the drawer considers to be an
     * equivalent item. For drawers that are empty, locking status is considered.
     *
     ...
     *
     * This is intended to allow outbound ore-dictionary conversions of compatible items, as defined by what the drawer
    ...
    /**
     * Gets whether or not the drawer has items. A drawer set with an item type and 0 count is not considered empty.
     */
    boolean isEmpty();
