...
public class InventoryUpgrade implements IInventory {
    ...
    public ItemStack decrStackSize(int slot, int count) {
        ...
        if (count > 0) tile.setUpgrade(slot, null);
    ...
    private boolean canRemoveStorageUpgrade(TileEntityDrawers tile, int storageLevel) {
        ...
        if (effectiveStorageMult == storageMult) storageMult--;
        ...
        for (int i = 0; i < tile.getDrawerCount(); i++) {
            ...
            if (drawer.getMaxCapacity() - addedItemCapacity < drawer.getStoredItemCount()) return false;
        }
