...
public final class UniqueMetaIdentifier {
    ...
    public UniqueMetaIdentifier(String compoundName) {
        ...
        if (parts1.length >= 2) this.meta = Integer.parseInt(parts1[1]);
        else if (parts2.length > 2) this.meta = Integer.parseInt(parts2[parts2.length - 1]);
        else this.meta = OreDictionary.WILDCARD_VALUE;
    }
    ...
    public GameRegistry.UniqueIdentifier getUniqueIdentifier() {
        if (cachedUID == null) cachedUID = new GameRegistry.UniqueIdentifier(modId + ":" + name);
        return cachedUID;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equal(modId, other.modId) && Objects.equal(name, other.name) && meta == other.meta;
    }
